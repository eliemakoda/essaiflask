...
def create_heatmap(data, filename):
    ...
    for parameter in ['wqi']:
        heat_data = [[barangay_coordinates[row['Barangay']][0], barangay_coordinates[row['Barangay']][1], row[parameter]] for _, row in data.iterrows() if row['Barangay'] in barangay_coordinates]
        ...
        folium.plugins.HeatMap(heat_data, name=parameter, min_opacity=0.5, gradient=gradient).add_to(m)
    ...
    <div style="
        ...
        ">
        &nbsp; Heatmap Legend <br>
        &nbsp; <i class="fa fa-square" style="color:#0000FF"></i> 0 - 25 &nbsp; Excellent <br>
        &nbsp; <i class="fa fa-square" style="color:#00FF00"></i> 26 - 50 &nbsp; Good <br>
        &nbsp; <i class="fa fa-square" style="color:#FFFF00"></i> 51 - 75 &nbsp; Poor <br>
        &nbsp; <i class="fa fa-square" style="color:#FFA500"></i> 76 - 100 &nbsp; Very Poor <br>
        &nbsp; <i class="fa fa-square" style="color:#FF0000"></i> > 100 &nbsp; Unsuitable <br>
    </div>
    """
...
def save_to_database():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            #app.logger.info("Month: %s, Year: %s", month, year)
            ...
            #app.logger.error("Error occurred while saving data: %s", str(e))
...
def update_dao_chart_data():
    ...
    for record in db_data:
        ...
        record_dict = {
            ...
            'Ammonia': float(record.Ammonia) if record.Ammonia is not None else None,
            'DO': float(record.DO) if record.DO is not None else None,
            'Nitrate': float(record.Nitrate) if record.Nitrate is not None else None,
            'Phosphate': float(record.Phosphate) if record.Phosphate is not None else None
        }
        db_data_dict.append(record_dict)
...
def create_header(pdf, selected_quarter, selected_year):
    ...
    pdf.cell(0, 4, 'Water Quality Updates in Taal Lake as of ' + formatted_date, 0, 1, 'C')
...
def generate_report():
    ...
    if selected_year <= 2022:
        ...
        filtered_river_df = river_df[(river_df['Month'] == selected_month_number) & (river_df['Year'] == selected_year)]
    ...
    for i in range(len(param_list)):
        ...
        if(i == 0):
            for index, row in time_values.iterrows():
                pdf.cell(brgy_column,8,str(row['Time (AM)']),border=1, ln=False, align='C')
            pdf.cell(col_width,8,'','LTR', ln=False, align='C') 
        if(i == 1):
            for index, row in weather_values.iterrows():
                pdf.cell(brgy_column,8,str(row['Weather Condition']),border=1, ln=False, align='C')
            pdf.cell(col_width,8,'','LR', ln=False, align='C')
        if(i == 2):
            for index, row in wind_values.iterrows():
                pdf.cell(brgy_column,8,str(row['Wind Direction']),border=1, ln=False, align='C')
            pdf.cell(col_width,8,'','LR', ln=False, align='C')
        if(i == 3):
            for index, row in color_values.iterrows():
                pdf.cell(brgy_column,8,str(row['Color of Water (apparent)']),border=1, ln=False, align='C')
            pdf.cell(col_width,8,'','LR', ln=False, align='C')
        if(i == 4):
            for index, row in air_values.iterrows():
                pdf.cell(brgy_column,8, str(row['Air Temperature (0C)']),border=1, ln=False, align='C')
            pdf.cell(col_width,8,'','LR', ln=False, align='C')
        if(i == 5):
            for index, row in water_transparent_values.iterrows():
                pdf.cell(brgy_column,8, str(row['Water Transparency (m)']),border=1, ln=False, align='C')
            pdf.cell(col_width,8,'','LR', ln=False, align='C')
        # if(i == 6):
        ...
        if(i == 6):
            for index, row in water_temp_values.iterrows():
                pdf.cell(brgy_column,8, str(row['Water Temperature (0C)']),border=1, ln=False, align='C')
